...
- **Excepción acotada — tarjeta de clase, logo de marca y foto de galería:** en el carrusel de clases, el de marcas y la sección de galería (home / cualquier página), la imagen/logo sí escala levemente en hover/focus-visible (`transform: scale()`, ver Carrusel de clases / Carrusel de marcas / Foto con texto superpuesto más abajo) — el contenedor en sí no se mueve ni gana sombra, solo la imagen adentro. Es la única excepción a "nunca transform en hover" de esta sección, y no se extiende a la tarjeta de producto.
...
- Escritorio (≥1024px): una sola fila compacta en grid de 3 columnas — menú de clases a la izquierda, logo centrado, buscador + cuenta + carrito a la derecha.
...
### Menú de clases (escritorio)
Un botón (ícono de tres líneas + "Clases", sin caja) abre un panel flotante con la lista de clases del catálogo — reemplaza la fila de links en línea que había antes, que se veía apretada apenas el admin pasaba de 3-4 clases. Panel con sombra `float` (única reservada a menús/modales, ver Elevation & Depth), siempre sólido en Blanco sin importar que el header de atrás esté transparente (home) — grilla de hasta 2 columnas, links con el mismo subrayado dorado al hover que el resto del sitio. Se abre/cierra con clic, clic afuera, Escape o al elegir una clase; el ícono se tiñe de `gold-600` y rota levemente mientras está abierto. Distinto del panel de menú móvil (que sigue igual, sin tocar) — en móvil las clases ya vivían ahí, este menú nuevo es solo para escritorio.

### Flechas de carrusel (clases / marcas)
Sin caja: solo el ícono (18px) en `text-muted`, sin fondo ni borde en reposo — un círculo tenue (`rgba(32,30,31,.06)`) aparece recién en hover/foco, mismo criterio minimalista que el resto de íconos funcionales (`.nav-icon-btn`). Siempre en bucle (loop): seguir dando a la misma flecha vuelve al principio o al final en vez de deshabilitarse en la punta.
...
Vocabulario compartido entre el banner del home y la sección de galería — cualquier foto administrable puede llevar título, subtítulo y botón encima, con controles por foto: **posición del texto** (izquierda/centro/derecha, mueve el bloque de texto entero; la galería además tiene arriba/medio/abajo en el eje vertical), **oscurecido** (0–100%, una capa `rgba(32,30,31,·)` entre la foto y el texto — reemplaza el alpha fijo que había al principio) y **desenfoque** (0–20px, `filter:blur()` sobre la imagen con un `scale(1.1)` para que el blur no deje un borde claro en el límite de la foto). El banner además tiene proporción fija (`aspect-ratio`, 8/3 escritorio · 4/5 móvil) con `object-fit:cover`: nunca se genera marco, se recorta la imagen si hace falta — decisión explícita del cliente sobre priorizar "sin marco" por encima de "sin recorte". Transición entre fotos del banner y entre mensajes de la barra de anuncios (cuando se elige "Deslizamiento"): empuje suave con `transform`, no un cambio brusco — la que sale se desliza hacia un lado mientras la que entra viene del otro, tomando siempre el camino más corto del círculo al volver de la última a la primera. Las fotos de galería escalan levemente en hover/focus-within, mismo criterio que las tarjetas de clase (ver excepción acotada en Cards / Containers). El banner además se navega arrastrando con el dedo o el mouse (Pointer Events, sigue el gesto en vivo y decide al soltar según qué tanto se arrastró) y con flechas — círculo translúcido sobre la foto, igual que las del carrusel de la galería. La galería tiene además proporción de foto configurable por sección (`image_ratio_w`/`image_ratio_h`, ancho:alto — vacío deja el recorte por defecto de cada modo: 4/5 en grilla, 4/3 móvil · 4/5 escritorio en comparar, el alto natural en foto única/carrusel) y espaciado de la sección (`spacing`: normal / compact / flush, ver Layout) — mismo control que el carrusel de clases.
...
### Carrusel de clases
Sección administrable (`classes_carousel`) que muestra las clases (categorías) del catálogo como tarjetas de foto — todas las activas o una selección manual ordenable, leídas en vivo de la pestaña Clases del panel, sin duplicar datos en la sección. Tarjeta con proporción configurable (`card_ratio_w`/`card_ratio_h`, ancho:alto — 3/4 por defecto; más chico el segundo número, más fina queda la foto) y `object-fit:cover`, radio 0 en la imagen y 2px en la tarjeta; texto superpuesto (eyebrow en Jost 14px/500 y nombre en Newsreader ≥28px/500, sin mayúsculas sostenidas) sobre una capa `rgba(32,30,31,·)` graduable por clase (`overlay_darkness`, 0–100) más `blur` (0–20px, `filter:blur()` con `scale(1.1)` para que no deje un borde claro) y posición del texto — los mismos tres controles de banner/galería, ahora también acá. Indicador circular con borde 1px `gold-400` y flecha, que se llena de `gold-400` con flecha Ónix en hover — mismo criterio transitorio que `button-onDark`. En hover/focus-visible la imagen (no la tarjeta) escala a ~1.06 con `cubic-bezier(0.16,1,0.3,1)` en 500ms; sin animación bajo `prefers-reduced-motion`. Toda la tarjeta es un único `<a>` al catálogo filtrado por esa clase (`/catalogo.html?category_id=`), sin enlaces anidados. Pista con `scroll-snap-type:x mandatory` (deslizamiento nativo al dedo en móvil) y flechas siempre en bucle — seguir dando al mismo lado vuelve al principio o al final en vez de deshabilitarse en la punta — cantidad de tarjetas visibles configurable por pantalla (móvil/tablet/escritorio), por defecto 1.3/3/4 (el .3 asoma la siguiente tarjeta a propósito, para invitar al deslizamiento). Dos anchos de bloque: **contenido** (de siempre, dentro del `container` de 1200px) o **ancho completo** (la pista rompe el container y toca los bordes de la pantalla — el título se queda adentro, como cualquier título de sección; las flechas pasan a llevar el mismo fondo translúcido que las del banner, porque ahí sí quedan sobre la foto de la punta). Espaciado de la sección configurable (`spacing`: normal / compact / flush, ver Layout).
