...
- El buscador de móvil (`.nav-search-mobile input`, la franja aparte del menú hamburguesa) tenía `font-size: 15px` — por debajo de los 16px que Safari/iOS necesita para no hacer zoom automático al enfocar el campo. Ese zoom no se deshacía solo al cerrar el buscador, dejando la página alejada hasta que el visitante hacía zoom out a mano. Mismo arreglo que ya se le había hecho al input del chat: sube a 16px, sin cambios en escritorio.

- Tres ajustes al widget de chat en móvil, todos alrededor del teclado nativo: (1) abrir el panel ya no enfoca el input automáticamente — antes eso disparaba el teclado apenas se tocaba la burbuja, sin que el visitante hubiera tocado el campo; ahora el teclado solo aparece si tocan la barra de escribir, como cualquier input normal. (2) Mientras el teclado está activo, el panel se reancla al `visualViewport` real (`window.visualViewport`, con una heurística simple de cuánto se achicó respecto a `window.innerHeight`) en vez de depender de un `vh` fijo — en iOS Safari el layout viewport no se achica con el teclado, así que antes el panel quedaba mal ubicado o tapado y no se alcanzaban a leer los mensajes; al cerrarse el teclado vuelve solo al estilo de hoja fija normal. (3) El input pasa de `font-size: 14px` a `16px` solo en el breakpoint móvil — por debajo de 16px, Safari/iOS hace zoom automático al enfocar el campo, y ese zoom corría el botón de enviar fuera de la pantalla visible.
...
.
├── backend/
│   ├── routes/          # auth, products, categories, brands, orders, payments, settings, stats, page_sections, contact_messages, chat
...
│   ├── services/           # Wompi, Mercado Pago, email, pricing (descuentos), chat (n8n) y chat_catalog (contexto de catálogo y verificación de enlaces del chatbot)
│   ├── middleware/           # Auth (JWT) y dependencias de rol
│   ├── alembic/                # Migraciones de base de datos
│   └── tests/                   # pytest, SQLite en memoria (195 tests)
├── n8n/               # system-message.txt: el system message del chatbot, para copiarlo al nodo del AI Agent
...
| `FRONTEND_URL` | Orígenes del frontend, separados por coma, permitidos por CORS. El primero es el canónico: con él se arman el enlace del correo de recuperación de contraseña y las URL de retorno de Wompi y Mercado Pago |
...
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` | Opcionales. Bot y chat que reciben el aviso por Telegram cuando un pedido pasa a pagado; si falta alguna no se avisa y solo queda un log |
